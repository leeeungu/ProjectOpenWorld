...
void UPalPatternComponent::StartPattern(uint8 PatternIndex)
{
	if (PatternClassArray.IsValidIndex(PatternIndex))
	{
		...
		{
			IAttackInterface* OwnerCharacter = Cast< IAttackInterface>(GetOwner());
			TObjectPtr<UPalAttackComponent> AttackComp = OwnerCharacter ? OwnerCharacter->GetPalAttackComponent() : nullptr;
